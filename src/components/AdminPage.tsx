...
const UsersTab: React.FC<{ jwt: string; user: UserInfo }> = ({ jwt, user }) => {
  ...
  async function handleAdd(e: React.FormEvent) {
    e.preventDefault();
    if (newPassword !== newPasswordConfirm) { setError('הסיסמאות אינן תואמות'); return; }
    ...
    try {
      ...
      setNewPassword(''); setNewPasswordConfirm(''); setNewRole('hevrutah');
      setShowForm(false);
      fetchUsers();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'שגיאה');
  ...
  function openEdit(u: AppUser) {
    ...
    setShowPasswordChange(false); setEditPassword(''); setEditPasswordConfirm('');
  }

  async function handleEdit(e: React.FormEvent) {
    e.preventDefault();
    if (editPassword && editPassword !== editPasswordConfirm) { setError('הסיסמאות אינן תואמות'); return; }
    if (!editUser) return;
    ...
    try {
      ...
      const res = await fetch(`/api/auth/users?id=${editUser.id}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
      const data = await res.json();
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div>
              ...
              <input
                ...
                value={newPasswordConfirm} onChange={e => setNewPasswordConfirm(e.target.value)} required placeholder="הכנס שוב את הסיסמה"
              />
            </div>
          </div>
      ...
      {editUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, direction: 'rtl' }}>
          <form noValidate onSubmit={handleEdit} style={{ background: 'white', borderRadius: 12, padding: 28, width: 420, maxWidth: '95vw', display: 'flex', flexDirection: 'column', gap: 14, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 3 }}>תפקיד</label>
              <select style={inputStyle} value={editRole} onChange={e => setEditRole(e.target.value as 'admin' | 'hevrutah' | 'external')} disabled={editUser.username === user.username}>
                <option value="hevrutah">מטפל חברותא</option>
                <option value="external">מטפל חיצוני</option>
                <option value="admin">מנהל</option>
              </select>
            ...
            {user.isAdmin && (
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: 12 }}>
                {!showPasswordChange ? (
                  <button type="button" onClick={() => setShowPasswordChange(true)} style={{ padding: '7px 14px', background: '#f8fafc', color: '#374151', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
                    🔑 שינוי סיסמה
                  </button>
                ) : (
                  <>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, direction: 'rtl' }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 28, width: 340, maxWidth: '90vw', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={() => setDeleteConfirm(null)} style={{ padding: '9px 22px', background: 'white', color: '#374151', border: '1px solid #d1d5db', borderRadius: 7, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
                ביטול
              </button>
              <button onClick={confirmDelete} style={{ padding: '9px 22px', background: '#dc2626', color: 'white', border: 'none', borderRadius: 7, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                כן, מחק
              </button>
            </div>
          </div>
